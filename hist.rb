...
class SaveFromCpeeStream < Riddl::Implementation #{{{
  def response
    ...
    if(Dir.exist?(File.join(__dir__,'history',uuid)).!()) then
      FileUtils.mkdir(File.join(__dir__,'history',uuid))
    end
    pp uuid
    pp type
    pp topic
...
class GetInstances < Riddl::Implementation #{{{
  def response
    return Riddl::Parameter::Complex.new("json","application/json",{:instances => Dir.children(File.join(__dir__,'history')).filter() { |el| el.start_with?('.').!() }.sort() { |a,b| a <=> b }}.to_json())
  end
end #}}}

class GetCommits < Riddl::Implementation #{{{
  def response
    history = File.join(__dir__,'history')
...
Riddl::Server.new(File.dirname(__FILE__) + '/hist.xml') do |opts|
  
  on resource do
    run SaveFromCpeeStream if post
    on resource 'instances' do
      run GetInstances if get
      on resource '[a-zA-Z\d_-]+' do
        on resource 'commits' do
          run GetCommits if get
          on resource '[a-zA-Z\d_-]+' do
